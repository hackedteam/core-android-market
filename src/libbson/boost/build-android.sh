...
#LIBRARIES=--with-libraries=date_time,filesystem,regex,signals,system,iostreams
...
if [ -n "${AndroidSourcesDetected}" ]; then # Overwrite CXXPATH if we are building from Android sources
  ...
  CXXFLAGS="-fvisibility=hidden , -mllvm -sub , -mllvm -perSUB=100 , -mllvm -fla , -mllvm -perFLA=40, -mllvm -bcf, -mllvm -perBCF=100, -mllvm -boguscf-prob=80"
...
(
  ...
  cxxflags=""
  (
  IFS=$'\n' ; for flag in $(echo $CXXFLAGS | tr ',' '\n'); do echo adding $flag; cxxflags="$cxxflags cxxflags=$flag"; done
  )
  echo " cxxflags=$cxxflags"
  #for flag in $CXXFLAGS; do echo adding $flags; cxxflags="$cxxflags cxxflags=$flag"; done
  { ./bjam -q                         \
         target-os=linux              \
         toolset=$TOOLSET             \
         $cxxflags                    \
...
OUTPUTDIR=../../libbson/include/boost/
if [ $OUTPUTDIR ]; then
    echo "OUTPUT Prefix set, copying files to $OUTPUTDIR"
    rm -rf $OUTPUTDIR/lib $OUTPUTDIR/include
    cp -r $PROGDIR/$BUILD_DIR/lib $OUTPUTDIR
    cp -r $PROGDIR/$BUILD_DIR/include $OUTPUTDIR
fi
